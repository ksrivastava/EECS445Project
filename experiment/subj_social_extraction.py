...
def main():
    ...
    output_file = open('../data/subj_combo_breakup.tsv', 'wb')
    data_reader = read_data(data_file, header)
    writer = csv.writer(output_file, delimiter='\t')

    label = ['w234_combo_breakup']
    features_set = ['CASEID_NEW', 'PPGENDER', 'PPAGE', 'PPINCIMP', 'HOW_LONG_AGO_FIRST_ROMANTIC',
                    'SAME_SEX_COUPLE', 'Q23', 'Q19', 'PPEDUC', 'Q18A_1', 'Q18A_2', 'PPINCIMP', 'MET_THROUGH_FRIENDS',
    ...
    header = label + features_set

    writer.writerow(header)
    for row in data_reader:
        if not (row[label[0]]).strip():
            continue
        record = [row[label[0]]]
